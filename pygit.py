...
def commitGit(file_address,origin_url):
    ...
    pys=[]
    for folderName, subfolders, filenames in os.walk(file_address):#对文件夹进行遍历
    ...
    #filesname=
    os.system(r'git pull origin main:main')#拉取

    for address in pys:
        repo.index.add(address)#逐一添加
        logging.debug(address+' has been added!')
    t = time.localtime()
    repo.index.commit('time:'+time.asctime(t)+' author:Chan_Leo')

    os.system(r'git push origin main:main')#上传

commitGit(r'D:\data\project\python',r'git@github.com:c369578336/python.git')
"""
